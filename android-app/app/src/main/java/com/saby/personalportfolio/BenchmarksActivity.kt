...
class BenchmarksActivity : AppCompatActivity() {
    ...
    private fun reload() {
        ...
        bindQuickAddChips(benchmarks, portfolioJson)
    ...
    private fun bindQuickAddChips(existing: List<Benchmark>, portfolioJson: String) {
        ...
        // Replaces the earlier separate search-and-resolve dialog
        // ("proxy fund" benchmarks) - that dialog's own network
        // resolve (a full mfapi.in scheme-list fetch) was a confirmed
        // real source of slowness/instability. This chip instead lists
        // funds ALREADY tracked normally (via the ordinary, already-
        // reliable Additional Funds / CAS import path) and flagged
        // "usable as benchmark" in Manage Names - adding one is a pure
        // local copy (AddBenchmarkFromAsset), no network call at all.
        val existingProxyISINs = existing.filter { it.proxyFundISIN.isNotEmpty() }.map { it.proxyFundISIN }.toSet()
        val nameListJson = Bridge.computeNameList(portfolioJson)
        val eligibleFunds: List<NameListEntry> = if (!isBridgeError(nameListJson)) {
            val entryType = object : TypeToken<List<NameListEntry>>() {}.type
            try {
                gson.fromJson<List<NameListEntry>>(nameListJson, entryType)
                    ?.filter { !it.isBenchmark && it.usableAsBenchmark && it.isin.isNotBlank() && it.isin !in existingProxyISINs }
                    ?: emptyList()
            } catch (e: Exception) {
                emptyList()
            }
        ...
        if (eligibleFunds.isNotEmpty()) {
            val chip = Chip(this)
            chip.text = "Add from tracked funds"
            chip.isClickable = true
            chip.setOnClickListener { showTrackedFundPicker(eligibleFunds) }
            quickAddGroup.addView(chip)
        }
    }

    /**
     * Instant picker over funds already flagged "usable as benchmark"
     * (toggled from Manage Names) - no search box, no network call.
     * Each entry already has its ISIN and NAV history known locally,
     * since it's a fund the person is already tracking the normal way.
     */
    private fun showTrackedFundPicker(eligibleFunds: List<NameListEntry>) {
        val names = eligibleFunds.map { NicknameResolver.resolve(it.name, it.nickname) }.toTypedArray()
        AlertDialog.Builder(this)
            .setTitle("Add benchmark from tracked funds")
            .setItems(names) { _, index -> addBenchmarkFromAsset(eligibleFunds[index]) }
            .setNegativeButton("Cancel", null)
            .show()
    }

    private fun addBenchmarkFromAsset(entry: NameListEntry) {
        val portfolioPath = PortfolioStorage.filePath(this)
        val portfolioJson = PortfolioLoadCache.load(portfolioPath)
        ...
        Toast.makeText(this, "Added ${entry.name} as a benchmark", Toast.LENGTH_SHORT).show()
        reload()
    }
